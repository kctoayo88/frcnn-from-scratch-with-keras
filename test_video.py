...
para = {'test_path': 'C:\\Users\\kctoa\\Desktop\VScode\\frcnn-from-scratch-with-keras\\input',
        'load': 'C:\\Users\\kctoa\\Desktop\\VScode\\frcnn-from-scratch-with-keras\\models\\resnet50\\pokemon.hdf5',
        'network': 'resnet50', 'config_filename': 'config.pickle', 'num_rois': 32, 'write': True}

if not para['test_path']:   # if filename is not given
...
if para['load'] == None:
  ...
  print('Loading weights from {}'.format(para['load']))
...
for idx, img_name in enumerate(files):
    ...
    if para['write'] == True:
           ...
           if not os.path.isdir("output"):
              os.mkdir("output")
           cv2.imwrite('./output/{}.jpg'.format(idx),img)