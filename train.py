...
if not os.path.isdir("models"):
  os.mkdir("models")
if not os.path.isdir("models/" + para['network']):
  os.mkdir(os.path.join("models", para['network']))
...
all_imgs, classes_count, class_mapping = get_data(para['train_path'], para['cat'])
...
# optimizer setup
if para['optimizers'] == "SGD":
    if para['rpn_weight_path'] is not None:
        optimizer = SGD(para['lr']/100, decay = 0.0005, momentum = 0.9)
        optimizer_classifier = SGD(lr = para['lr']/5, decay = 0.0005, momentum = 0.9)
    else:
        optimizer = SGD(lr = para['lr']/10, decay = 0.0005, momentum = 0.9)
        optimizer_classifier = SGD(lr = para['lr']/10, decay = 0.0005, momentum = 0.9)
else:
    optimizer = Adam(lr=para['lr'], clipnorm = 0.001)
    optimizer_classifier = Adam(lr = para['lr'], clipnorm = 0.001)
...
if para['load'] is not None:
    print("loading previous model from ", para['load'])
    model_rpn.load_weights(para['load'], by_name = True)
    model_classifier.load_weights(para['load'], by_name = True)
elif para['rpn_weight_path'] is not None:
    print("loading RPN weights from ", para['rpn_weight_path'])
    model_rpn.load_weights(para['rpn_weight_path'], by_name = True)
else:
...
epoch_length = int(para['epoch_length'])
...
for epoch_num in range(num_epochs):
    ...
    if epoch_num == 3 and para['rpn_weight_path'] is not None:
        K.set_value(model_rpn.optimizer.lr, para['lr']/30)
        K.set_value(model_classifier.optimizer.lr, para['lr']/3)
